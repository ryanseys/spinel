...
void emit_cond(Compiler *c, int id, Buf *b) {
  ...
  if (t == TY_STRING || ty_is_array(t) || ty_is_hash(t) || ty_is_object(t) ||
      ...
      t == TY_BIGINT || t == TY_REGEX || t == TY_CURRY || t == TY_FIBER || t == TY_THREAD || t == TY_QUEUE || t == TY_MUTEX || t == TY_CONDVAR || t == TY_RANDOM || t == TY_DIR ||
      ...
      if (pred >= 0) {
        ...
        if (nt_type(nt, conds[j]) && sp_streq(nt_type(nt, conds[j]), "SplatNode")) {
          ...
          if (reidx >= 0 && pt == TY_STRING) {
            ...
            else buf_printf(b, "; sp_frange_cover(_t%d, (mrb_float)_t%d); })", tr, t);
          }
    ...
    if (sp_streq(name, "slice!") && assignable2) {
      ...
      if (argc == 1 && (comp_ntype(c, argv[0]) == TY_INT || comp_ntype(c, argv[0]) == TY_RANGE)) {
        ...
        if (comp_ntype(c, argv[0]) == TY_RANGE) {
          ...
          buf_printf(b, "; mrb_int _t%d = (mrb_int)sp_str_length(", tn2); emit_expr(c, recv, b);
          ...
          buf_printf(b, "; mrb_int _t%d = 1; mrb_int _t%d = (mrb_int)sp_str_length(", tl2, tn2);
          emit_expr(c, recv, b);
