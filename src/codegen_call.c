...
static void emit_poly_fetch_absent(Compiler *c, int argc, const int *atmp, int keynode,
                                   int argv1, TyKind ret, TyKind trt, Buf *b) {
  if (argc == 2) {
    ...
    /* KeyError carries the missing key's inspect form; box the raw key temp. */
    char kx[32]; snprintf(kx, sizeof kx, "_t%d", atmp[0]);
    TyKind kt = keynode >= 0 ? infer_type(c, keynode) : TY_POLY;
    Buf kb; memset(&kb, 0, sizeof kb);
    if (kt == TY_POLY) buf_puts(&kb, kx); else emit_boxed_text(c, kt, kx, &kb);
    buf_printf(b, "(sp_raise_key_not_found(%s), ", kb.p ? kb.p : "sp_box_nil()");
    ...
    if (ncand > 0 || is_index || is_include || is_fetch || is_push || is_pred) {
      ...
      if ((is_aref || is_fetch) && infer_type(c, argv[0]) == TY_STRING) {
        ...
        for (unsigned hvi = 0; hvi < sizeof HV / sizeof HV[0]; hvi++) {
          ...
          else if (is_fetch) { buf_printf(b, "(sp_raise_key_not_found(sp_box_str(_t%d)), ", atmp[0]); buf_puts(b, ret == TY_POLY ? "sp_box_nil()" : default_value(trt)); buf_puts(b, ")"); }
        ...
        if (is_aref || is_fetch) {
          ...
          if (is_fetch) { buf_puts(b, " : "); emit_poly_fetch_absent(c, argc, atmp, argv[0], argc == 2 ? argv[1] : -1, ret, trt, b); }
          buf_puts(b, "; break;");
        }
      }
      ...
      if ((is_aref || is_fetch) &&
        ...
        if (is_fetch) { buf_puts(b, " : "); emit_poly_fetch_absent(c, argc, atmp, argv[0], argc == 2 ? argv[1] : -1, ret, ptrt, b); }
        buf_puts(b, "; break;");
      }
