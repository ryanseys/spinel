...
static void emit_ffi_call_from_func(Compiler *c, int fi, int argc, const int *argv, Buf *b) {
  ...
  const char *fmod = c->ffi_funcs[fi].mod;
  /* A function taking an ffi_callback has its extern skipped (codegen.c): the
     symbol is declared by a system header whose per-argument const
     qualification we can't reproduce, so we call the header prototype directly.
     Pointer-data args then carry our own (const) element types; cast them to
     void* so the implicit conversion to the header's real parameter type is
     warning-free under -Werror on clang and gcc. */
  int hdr_call = 0;
  for (int hi = 0; hi < call_argc; hi++)
    if (ffi_find_callback(c, fmod, c->ffi_funcs[fi].args[hi]) >= 0) { hdr_call = 1; break; }
  ...
  for (int ai = 0; ai < call_argc && ai < argc; ai++) {
    ...
    int cbidx = ffi_find_callback(c, fmod, spec);
    ...
    int voidp = hdr_call && (sp_streq(spec, "str") || sp_streq(spec, "int_array") ||
                             sp_streq(spec, "float_array")) && !is_fptr;
...
void emit_call(Compiler *c, int id, Buf *b) {
  ...
  if (recv >= 0) {
    ...
    if (rcmod) {
      ...
      if (fi >= 0) {
        emit_ffi_call_from_func(c, fi, argc, argv, b);
        return;
      }
