...
static mrb_int sp_int_bit_range(mrb_int n, mrb_int start, mrb_int len) {
  ...
  /* start <= -64 first, so `-start` is never computed for MRB_INT_MIN (which
     would overflow); a shift that wide drops every bit, leaving 0. */
  else shifted = (start <= -64) ? 0 : (mrb_int)((uint64_t)n << -start);
...
/* Math.lgamma(x) -> [log(|gamma(x)|), sign of gamma(x)], matching CRuby:
   +Infinity -> [Infinity, 1], -Infinity raises Math::DomainError, NaN passes
   through as [NaN, 1], and every non-positive integer is a pole -> [Infinity, 1]
   (detected exactly via floor(), since sin(pi*x) is only near-zero there). For
   the remaining x < 0 the reflection formula gives the value and sign. */
static sp_PolyArray *sp_math_lgamma(double x) {
  int sign = 1; double v;
  if (x != x) v = x;                       /* NaN -> [NaN, 1] */
  else if (x == INFINITY) v = INFINITY;    /* [Infinity, 1] */
  else if (x == -INFINITY)
    sp_raise_cls("Math_DomainError", "Numerical argument is out of domain - lgamma");
  else if (x <= 0.0 && x == floor(x)) v = INFINITY;  /* non-positive integer pole */
  else if (x > 0.0) v = sp_lgamma_pos(x);
  else {
    double s = sin(M_PI * x);
    if (s < 0.0) sign = -1;
