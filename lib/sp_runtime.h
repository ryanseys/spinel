...
/* Complex#real / #imaginary keeping the component's CRuby class: an exact
   Rational component (a Rational entered the arithmetic) boxes as a Rational,
   otherwise the Integer/Float split of sp_complex_comp_v applies. */
static sp_RbVal sp_complex_comp_x(sp_Complex a, int im) {
  if (a.exact & (im ? SP_CPLX_IM_X : SP_CPLX_RE_X))
    return sp_box_rational(im ? a.im_r : a.re_r);
  return sp_complex_comp_v(im ? a.im : a.re, a.fl & (im ? SP_CPLX_IM_F : SP_CPLX_RE_F));
}
...
static mrb_float sp_poly_to_f(sp_RbVal v) { if (v.tag == SP_TAG_FLT) return v.v.f; if (v.tag == SP_TAG_INT || v.tag == SP_TAG_SYM) return (mrb_float)v.v.i; if (v.tag == SP_TAG_BIGINT) return sp_bigint_to_double((sp_Bigint *)v.v.p); if (sp_poly_is_rational(v)) return sp_rational_to_f(*(sp_Rational *)v.v.p); if (v.tag == SP_TAG_STR) return (mrb_float)atof(v.v.s ? v.v.s : sp_str_empty); if (v.tag == SP_TAG_BOOL) return v.v.b ? 1.0 : 0.0; return 0.0; }  /* STR arm mirrors sp_poly_to_i's strtoll and the typed String#to_f (atof); a boxed Rational (e.g. Complex#real) coerces exactly */
