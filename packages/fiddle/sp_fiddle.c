...
/* p[off, len] -> a String of len bytes (binary-safe: the returned spinel string
   ...
   Access is unchecked against `size`, exactly like CRuby's Fiddle::Pointer:
   `p[0, 20]` on an 8-byte buffer reads 20 raw bytes -- Fiddle is the raw-FFI
   escape hatch. A negative offset is the one deliberate divergence: it is never
   a valid Fiddle call, so we return empty rather than perform a wild read
   (mirroring the intentionally-safer idempotent #free). */
...
/* p[off, len] = str: copy min(len, str.bytesize) bytes into the buffer.
   Unchecked against `size` like CRuby; negative offset is a no-op (see slice). */
...
/* #size: the byte size recorded at malloc (0 for a wrapped raw pointer). */
mrb_int sp_FiddlePtr_size(sp_FiddlePtr *fp) { return fp ? (mrb_int)fp->size : 0; }
