...
| `o.extend(M)` on an instance | supported for stateless module methods | `o.extend(M)` where `o` is a local/param and `M` a static module makes `o` respond to M's instance methods (a synthesized `ObjectExt_<M>` class holds them); `extend` returns self, multiple extends union the method sets. Loud compile rejects: a module method that touches an instance variable (an extended bare `Object` has no ivar storage) and a call on the receiver *before* the extend (the static rewrite has no textual point to model per-point dispatch). A non-local receiver or a multi-module `extend(M, N)` call is left unhandled (the method call then raises `NoMethodError` as before) |
